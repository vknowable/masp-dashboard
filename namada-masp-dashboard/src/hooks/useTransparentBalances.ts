    ...
    queryFn: () => {
      ...
      return {
        balances: tokenSupplies.supplies.map((supply) => {
          ...
          const transparentBalances = intervals.reduce(
            (acc, interval) => {
              acc[interval] =
                supply.supplies[interval] !== null
                  ? Number(supply.supplies[interval]) -
                    Number(maspBalance.balances[interval])
                  : null;
              return acc;
            },
          ...
          const changeIntervals: ChangeInterval[] = [
            "24h",
            "7d",
            "30d",
            "allTime",
          ...
          const changes = changeIntervals.reduce(
            (acc, changeInterval) => {
              if (changeInterval === "allTime") return acc;
              const referenceInterval =
                changeInterval === "24h"
                  ? "1dAgo"
                  : (`${changeInterval}Ago` as BalanceInterval);
              acc[changeInterval] =
                transparentBalances.current !== null &&
                transparentBalances[referenceInterval] !== null
                  ? transparentBalances.current -
                    transparentBalances[referenceInterval]
                  : null;
              return acc;
            },
