...
import type { TokenDisplayRow, Token, TokensResponse } from '../types/token'
...
export function useMetricsData(): MetricsData {
  ...
  // Get token list
  const { data: tokenList = [] as Token[], isLoading: isLoadingTokens } = useTokenList()
  ...
  const tokens = Array.isArray(tokenList) ? tokenList.reduce<TokenDisplayRow[]>((acc, token) => {
    // Find matching asset in registry
    const matchingAsset = registryData?.assetList?.assets?.find(
    ...
    const tokenData: TokenDisplayRow = {
      ...
      usdPrice: 0, // Will be updated with real data
    }
  ...
  return {
    ...
    isLoading: isLoadingRegistry || isLoadingMasp || isLoadingTokens || isLoadingSupplies || isLoadingEpoch || isLoadingMaspBalances,
    error: registryError
  }
} 