        ...
        queryFn: async () => {
            ...
            return {
                ...
                supplies: data.supplies.map(token => {
                    const current = token.supplies.current ? parseInt(token.supplies.current) : null;
                    const oneDayAgo = token.supplies['1dAgo'] ? parseInt(token.supplies['1dAgo']) : null;
                    const sevenDaysAgo = token.supplies['7dAgo'] ? parseInt(token.supplies['7dAgo']) : null;
                    const thirtyDaysAgo = token.supplies['30dAgo'] ? parseInt(token.supplies['30dAgo']) : null;
