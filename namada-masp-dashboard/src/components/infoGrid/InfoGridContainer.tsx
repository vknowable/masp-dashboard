...
import { denomAmount, formatMagnitude, formatNumber, formatPercentage } from "../../utils/numbers"
...
function InfoGridContainer() {
    ...
    const { metrics: chainMetrics, isLoading: isLoadingChain, isError: isErrorChain } = useChainInfo()
    const cards = useMemo(() => chainMetrics ? createInfoCards(chainMetrics) : [], [chainMetrics])
...
function createInfoCards(chainMetrics: ChainMetrics): InfoCard[] {
    ...
    return [
        ...
        {
            topText: "Staking APR",
            bottomText: chainMetrics.stakingApr !== null ? `${formatPercentage(chainMetrics.stakingApr * 100)}` : '--',
            size: 'small' as const,
            variant: 'secondary' as const
        },
        {
            topText: "Total native supply",
            bottomText: chainMetrics.totalSupply && chainMetrics.totalSupply !== null
        ...
        },
        {
            ...
            bottomText: chainMetrics.percentStaked !== null
                ? `${formatNumber(chainMetrics.totalStaked)} (${formatPercentage(chainMetrics.percentStaked)})`
                : '--',
            size: 'small' as const,
            variant: 'secondary' as const
        },
