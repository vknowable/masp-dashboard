...
const GITHUB_REPO = import.meta.env.VITE_REGISTRY_REPO || "vknowable/mock-registry";
const GITHUB_BRANCH = import.meta.env.VITE_REGISTRY_BRANCH || "main";
...
async function fetchRegistryJson<T>(url: string): Promise<T | null> {
    try {
        ...
        console.error(
            `Error fetching Registry Json from: ${url}`,
...
async function fetchIbcFileList(url: string): Promise<IbcFileList> {
    try {
        ...
        console.error(
            `Error fetching Registry IBC files from: ${url}`,
            error instanceof Error ? error.message : "Unknown error",
        );
        return getEmptyIbcFileList();
    }
...
export async function fetchChainMetadata(
    ...
    try {
        const [chainJson, assetListJson] = await Promise.all([
            ...
            fetchRegistryJson<RegistryAssetList>(
                `${REPO_URL}/${name}/assetlist.json`,
            ),
        ]);
            ...
            matchingFiles.map(async (file) => {
                ...
                if (fileData) {
                    ...
                    const [counterPartyChainJson, counterPartyAssetListJson] =
                        await Promise.all([
                            fetchRegistryJson<RegistryChainJson>(
                                `${COSMOS_REGISTRY_URL}/${counterPartyChainName}/chain.json`,
                            ),
                            fetchRegistryJson<RegistryAssetList>(
                                `${COSMOS_REGISTRY_URL}/${counterPartyChainName}/assetlist.json`,
                            ),
                        ]);
